...
//  tree->SetBranchAddress("reco_muon_isTriggerMatched", reco_muon_isTriggerMatched, &b_reco_muon_isTriggerMatched);
  ...
  if (mass_barrel_h->Integral() < 0.999 || mass_barrel_h->Integral() > 1.001) mass_barrel_h->Scale(1./mass_barrel_h->GetEntries());
  if (mass_endcap_h->Integral() < 0.999 || mass_endcap_h->Integral() > 1.001) mass_endcap_h->Scale(1./mass_endcap_h->GetEntries());

  if (pt_res_barrel_h->Integral() < 0.999 || pt_res_barrel_h->Integral() > 1.001) pt_res_barrel_h->Scale(1./pt_res_barrel_h->GetEntries());
  if (pt_res_intermediate_h->Integral() < 0.999 || pt_res_intermediate_h->Integral() > 1.001) pt_res_intermediate_h->Scale(1./pt_res_intermediate_h->GetEntries());
  if (pt_res_endcap_h->Integral() < 0.999 || pt_res_endcap_h->Integral() > 1.001) pt_res_endcap_h->Scale(1./pt_res_endcap_h->GetEntries());
