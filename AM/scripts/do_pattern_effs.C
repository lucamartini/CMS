...
void do_pattern_effs(std::string filename, int sec_num, int nh)
{
  ...
  TEfficiency* pt_Eff = new TEfficiency("pt_Eff","pt efficiency;#p_{T};#epsilon",100,0.,100.);
  TEfficiency* pt_Eff_z = new TEfficiency("pt_Eff_z","pt efficiency;#p_{T};#epsilon",50,0.,10.);
...
//    if (fabs(eta)>2.15) continue;
    ...
    {
      for (unsigned int jj=0;jj<overlap_evt.size();++jj)
      {
      ...
      {
        for (unsigned int jj=0;jj<overlap_evt_good.size();++jj)
        {
    ...
    if (already_known==false) 
    {
      ...
      pt_Eff->Fill(passed, pt);
      if (pt<10) pt_Eff_z->Fill(passed_z, pt);
    }         
...
//  char buffer[80];
  ...
  TCanvas* c1 = new TCanvas("c1","Overall efficiencies",6,102,1526,921);
  gStyle->SetOptStat(0);
  gStyle->SetOptTitle(0);
...
//  pt_Eff->Draw("AP");
  ...
  TLegend* leg = new TLegend(0.6,0.5,0.85,0.65);
...
//  tracks->Draw("COLZ");
  ...
  TPaveText *pte_2 = new TPaveText(-0.57,0.61,0.58,0.68,"br");
  pte_2->SetFillColor(0);
  pte_2->SetTextFont(42);
  pte_2->SetTextSize(0.0349944);
  sprintf (buffer, "Tracks collected in the sector");  
//  TText *text_2 = pte_2->AddText(buffer);
  pte_2->Draw();
